...
public class ConnectionDtoService extends AbstractDtoService<Connection, CreateConnectionDto, ConnectionDto> {
    ...
    public List<ConnectionDto> findAllByMe(String connectionName) {
        return toDto(connectionService.findAllByMe(identityContextHolder.getIdentity().getAccessKeyId(),
                connectionName));
    }
